...
import BranchVehicles from './BranchVehicles';
...
const BranchConfig: React.FC<BranchConfigProps> = ({ onBack, initialData }) => {
    const { user } = useAuth();
    ...
    const performSave = async (silent = false): Promise<boolean> => {
        if (!formData.name.trim()) {
            if (!silent) alert('O nome da filial é obrigatório.');
            return false;
        }
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'profile':
                return <BranchForm data={formData} onChange={handleFieldChange} />;
            case 'vehicles':
                return <BranchVehicles branchId={formData.id} onEnsureExists={() => performSave(true)} />;
            default:
                return <BranchForm data={formData} onChange={handleFieldChange} />;
        }
    };
                ...
                <div className="px-8 py-5">
                    <div className="flex items-center gap-4 mb-1">
                        ...
                        <h1 className="text-xl font-bold text-blue-900">
                            {isNew ? 'Nova Filial' : formData.name}
                        </h1>
                    </div>
                    <div className="text-xs text-slate-400 ml-10">
                        Filiais / {isNew ? 'Cadastrar Filial' : 'Configurar Filial'}
                    </div>
                </div>
