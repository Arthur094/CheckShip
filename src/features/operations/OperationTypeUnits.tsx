...
interface OperationTypeUnitsProps {
...
const OperationTypeUnits: React.FC<OperationTypeUnitsProps> = ({ vehicleTypeId, onEnsureExists }) => {
    ...
    const handleToggle = async (vehicle: Vehicle) => {
        ...
        try {
            ...
            if (targetTypeId && vehicle.vehicle_type_id && vehicle.vehicle_type_id !== targetTypeId) {
                if (!confirm('Este veículo já está vinculado a outro Tipo de Operação. Deseja transferi-lo?')) return;
            }
    ...
    const handleSelectAll = async () => {
        ...
        if (!confirm(`Deseja vincular TODAS as ${filteredVehicles.length} placas exibidas a este Tipo de Operação?`)) return;
    ...
    const handleDeselectAll = async () => {
        if (!confirm(`Deseja desvincular TODAS as ${filteredVehicles.length} placas exibidas deste Tipo de Operação?`)) return;
...
export default OperationTypeUnits;
