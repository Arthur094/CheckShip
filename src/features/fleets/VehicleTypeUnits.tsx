...
interface VehicleTypeUnitsProps {
...
interface Filters {
    vehicleTypes: string[];
    active: string[];
    show: string[]; // 'Selected', 'NotSelected'
}

const VehicleTypeUnits: React.FC<VehicleTypeUnitsProps> = ({ vehicleTypeId, onEnsureExists }) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const { data: vData, error: vError } = await supabase
                .from('vehicles')
                .select('id, plate, model, brand, active, vehicle_type_id')
                .order('plate');
    ...
    const handleToggle = async (vehicle: Vehicle) => {
        let targetTypeId = vehicleTypeId;

        // If we are unlinking, set to null
        const isLinked = vehicle.vehicle_type_id === vehicleTypeId;
        if (isLinked) {
            targetTypeId = null;
        } else {
            // We are linking. Ensure Type exists first.
            if (!vehicleTypeId) {
                const saved = await onEnsureExists();
                if (!saved) return;
                // See note below about waiting for prop update
                return;
            }
        }

        try {
            // If linking to another type, confirm?
            if (targetTypeId && vehicle.vehicle_type_id && vehicle.vehicle_type_id !== targetTypeId) {
                if (!confirm('Este veículo já está vinculado a outro Tipo. Deseja transferi-lo?')) return;
            }

            const { error } = await supabase
                .from('vehicles')
                .update({ vehicle_type_id: targetTypeId })
                .eq('id', vehicle.id);

            if (error) throw error;

            // Optimistic update
            setVehicles(prev => prev.map(v => v.id === vehicle.id ? { ...v, vehicle_type_id: targetTypeId } : v));
    ...
    const handleToggleWrapper = async (vehicle: Vehicle) => {
        if (!vehicleTypeId) {
            ...
            if (!success) return;
            // Ideally we would trigger a refresh or handle linking after parent update
            return;
        }
    ...
    const filteredVehicles = useMemo(() => {
        return vehicles.filter(v => {
            ...
            // Vehicle Type Filter
            // If no filter selected, show all. If selected, check if vehicle's type is in list.
            // Note: vehicle_type_id might be null.
            ...
            // Show Filter (Linked/Unlinked relative to CURRENT type)
            // 'Selecionados' = Linked to current type
            // 'Não Selecionados' = Not linked to current type (could be null or other type)

            let matchesShow = true;
            if (filters.show.length > 0) {
                const isLinked = v.vehicle_type_id === vehicleTypeId;
    ...
    }, [vehicles, searchTerm, filters, vehicleTypeId]);
    ...
    const handleClearFilters = () => {
        setFilters({ vehicleTypes: [], active: [], show: [] });
    };
    ...
    const handleSelectAll = async () => {
        ...
        if (!confirm(`Deseja vincular TODAS as ${filteredVehicles.length} placas exibidas a este Tipo de Veículo?`)) return;
        ...
        try {
            ...
            // Optimistic
            setVehicles(prev => prev.map(v => idsToUpdate.includes(v.id) ? { ...v, vehicle_type_id: vehicleTypeId } : v));
    ...
    const handleDeselectAll = async () => {
        if (!confirm(`Deseja desvincular TODAS as ${filteredVehicles.length} placas exibidas deste Tipo de Veículo?`)) return;
        ...
        try {
            ...
            const { error } = await supabase
                ...
                .in('id', idsToUpdate); // Logic: we imply deselect means removing link? Or actually standard 'Select All' deselect.
            // Usually Deselect All in this context (Vincular column) means Unlink All.

            if (error) throw error;

            setVehicles(prev => prev.map(v => idsToUpdate.includes(v.id) && v.vehicle_type_id === vehicleTypeId ? { ...v, vehicle_type_id: null } : v));
            alert(`${idsToUpdate.length} veículos desvinculados!`);

        } catch (error: any) {
            alert('Erro ao desvincular: ' + error.message);
        } finally {
            setLoading(false);
            setShowSelectAll(false); // Hide the menu
        }
    };
            ...
            {hasActiveFilters && (
                <div className="bg-slate-100 px-8 py-3 border-b border-slate-200 flex items-center gap-2 flex-wrap">
                    ...
                    {Object.entries(filters).map(([key, value]) => {
                        ...
                        const label = key === 'vehicleTypes' ? 'Tipo' : key === 'active' ? 'Ativo' : 'Exibir';
                        return (
                ...
                <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-slate-100 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase rounded-t-lg">
                        ...
                        <div className="col-span-2 text-right relative group/header">
                            ...
                            >
                                ...
                                {/* Optional: Deselect All if needed, but space is tight. User asked for 'option to select all' */}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="divide-y divide-slate-100">
                        {filteredVehicles.map((vehicle) => {
                            const isLinked = vehicle.vehicle_type_id === vehicleTypeId;
                            const isLinkedOther = vehicle.vehicle_type_id && vehicle.vehicle_type_id !== vehicleTypeId;
                            ...
                            return (
                                <div key={vehicle.id} className={`grid grid-cols-12 gap-4 px-6 py-4 items-center transition-colors ${isLinked ? 'bg-blue-50/50' : 'hover:bg-slate-50'}`}>
                                    ...
                                    <div className="col-span-2 flex justify-end items-center gap-2">
                                        {/* 'Outro vínculo' badge REMOVED as requested */}
