...
const ChecklistConfig: React.FC = () => {
        ...
        <div className="space-y-4">
          {areas.map((area, areaIdx) => (
            <div key={area.id} className="border border-slate-200 rounded-2xl bg-white shadow-sm overflow-hidden" onClick={() => setActiveMenuId(null)}>
              ...
              {/* SubAreas List */}
              {area.subAreas && area.subAreas.map((subArea, subAreaIdx) => (
  ...
  const renderItemRow = (item: AreaItem, areaIdx: number, subAreaIdx: number | undefined, itemIdx: number) => (
    <div
      key={item.id}
