      ...
      <main className="flex-1 w-full max-w-md mx-auto px-4 pb-8 pt-2">
        ...
        <div className="py-4 sticky top-[60px] z-30 bg-background-light dark:bg-background-dark">
          <div className="relative group">
            ...
            <input
              type="text"
              className="block w-full rounded-xl border-0 py-3.5 pl-10 pr-4 text-slate-900 ring-1 ring-inset ring-slate-100 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-primary bg-white sm:text-sm sm:leading-6 shadow-sm transition-shadow"
        ...
        <div className="space-y-3">
          {filteredVehicles.map((vehicle) => (
            <button
              key={vehicle.id}
              onClick={() => vehicle.status === 'available' && navigate(`/template/${vehicle.plate}`)}
              disabled={vehicle.status !== 'available'}
              className={`w-full group relative flex items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-100 transition-all duration-200 text-left ${vehicle.status !== 'available' ? 'opacity-60 grayscale-[0.5] hover:opacity-100' : 'hover:shadow-md active:scale-[0.99]'}`}
            >
              <div className={`flex shrink-0 items-center justify-center size-14 rounded-lg ${vehicle.status === 'maintenance' ? 'bg-slate-50 text-slate-400' : 'bg-blue-50 text-primary'}`}>
              ...
              <div className="flex flex-col flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-slate-900 text-base font-bold leading-none truncate">
                    {vehicle.plate}
                  </p>
                ...
                <p className="text-slate-500 text-sm font-medium leading-tight truncate mb-0.5">
                  {vehicle.model}
                </p>
